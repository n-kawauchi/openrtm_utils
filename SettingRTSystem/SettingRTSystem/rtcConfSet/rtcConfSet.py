...
class RTComponentProfile():
    ...
    # 
    def __init__(self):
        global defaultCompPath
        self.compPath = defaultCompPath
    ...
    # @return ファイルパス
    # 
    def getFile(self, dirname, filename, compPath=defaultCompPath):
    ...
    # @return RTCプロファイル
    # 
    def getProfile(self, name, compPath=defaultCompPath):
...
class ConfDataInterface_i (RTCConfData__POA.ConfDataInterface):
    ...
    # 
    def saveConfigFile(self, comp ,filePath, saveFile, rtcconffile):
        ...
        s = name + ".config_file: " + os.path.relpath(compname, saveFile).replace("\\","/") + "\n"
        rtcconffile.write(s)
    ...
    def save(self, filename):
        ...
        work_dir = os.path.join(name, "workspace")
        ...
        self.saveData(f, self.confList_cpp, work_dir, dirname[2], False, compositeRTCList["C++"],"C++")
        ...
        try:
            ...
            for n in rtcs:
                ...
                if comp.name != "rtcdControl0.rtc":
                    ...
                    self.saveConfigFile(comp, dirname[2], work_dir, f)
                else:
        ...
        self.saveData(f, self.confList_py, work_dir, dirname[3], False, compositeRTCList["Python"],"Python")
        ...
        try:
            ...
            for n in rtcs:
                ...
                if comp.name != "rtcdControlPy0.rtc":
                    self.getConnectRTCs(comp,components)
                    self.saveConfigFile(comp, dirname[3], work_dir, f)
                else:
    ...
    def saveBatFile(self, home_dirname, cpp_dirname, py_dirname, sysfileName, compositeList):
        ...
        if not os.path.exists(fname):
            ...
            self.writeFileOption(f)
            if os.name == 'posix':
                cmd = "export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:" + defaultDllPath.replace("\\","/")+ "\n"
                f.write(cmd)
                cmd = "cdir=${PWD}" + "\n"
                f.write(cmd)
            elif os.name == 'nt':
            ...
            work_path = os.path.join(home_dirname, "workspace")
            if os.name == 'posix':
                cmd = "workspaceDir=" + os.path.relpath(work_path, home_dirname).replace("\\","/") + "\n"
                f.write(cmd)
                ...
                cmd = "set workspaceDir=" + os.path.relpath(work_path, home_dirname).replace("/","\\") + "\n"
                f.write(cmd)
            ...
            if os.name == 'posix':
                path = os.path.join(defaultPath, "Manager/Cpp/rtcd_p/rtcd_p")
                
                cmd = path + " -f " + os.path.relpath(cpp_dirname,work_path).replace("\\","/") + "/rtc.conf" + "&\n"
    ...
    def startRTCD_Cpp(self):
        ...
        if self.rtcdControlprocess:
            ...
            self.rtcdControlprocess = None
        global defaultRtcdControlPath
        f = open(self.cppDirName+"/rtc.conf", 'w')
        self.saveData(f, self.confList_cpp, ".", self.cppDirName, True, [], "C++")
        cmd = "TEST.rtcdControl0.config_file: " + str(os.path.join(defaultRtcdControlPath, "rtcdcontrol.conf")) + "\n"
        f.write(cmd)
    ...
    def startRTCD_Py(self):
        ...
        cmd = "TEST.rtcdControlPy0.config_file: " + str(os.path.join(defaultRtcdPyControlPath, "rtcdControlPy.conf")) + "\n"
        f.write(cmd)
    ...
    # @param compositeList 複合コンポーネントのリスト
    def saveData(self, fd, confList, savefile, filepath,  rtcdFlag=True,compositeList=[],language="Python"):
        global defaultCompositePath
        filename = ""
        if language == "C++":
            if os.name == 'posix':
                filename = searchFile("ECandStateSharedComposite.so",os.path.join(defaultCompositePath,"ECandStateSharedComposite"))
            elif os.name == 'nt':
                filename = searchFile("ECandStateSharedComposite.dll",os.path.join(defaultCompositePath,"ECandStateSharedComposite"))
        else:
            filename = searchFile("ECandStateSharedComposite.py",os.path.join(defaultCompositePath,"ECandStateSharedCompositePy"))
        filename = os.path.abspath(filename)
        dname = os.path.dirname(filename)
        fname = os.path.basename(filename)
        name, ext = os.path.splitext(fname)
        ...
        for d in confList:
            ...
            if rtcdFlag == False and d.id == "exec_cxt.periodic.filename" and d.data == "":
                ...
                of = open(text, "wb")
                of.close()
                text = os.path.relpath(text, savefile).replace("\\","/")
    ...
    def copyRTComponentDir(self, rtc_list, dir):
        for c in rtc_list:
            if c.num != 0:
                path1 = os.path.join(defaultCompPath,c.name)
